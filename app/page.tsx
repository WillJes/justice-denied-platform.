...
export default function Home() {
  ...
  return <main>
    ...
    <nav><a className="brand" href="#top"><b>JD</b> JUSTICE DENIED</a><button className="menu" onClick={()=>setMenu(!menu)}>MENU</button>
      <div className={menu ? "links open":"links"}>{[["The Story","story"],["Art & Justice","art"],["Watch","watch"],["Archive","archive"],["Festival Status","festivals"],["Speaking","speak"],["Donate","support"]].map(x=><a key={x[1]} href={"#"+x[1]} onClick={()=>setMenu(false)}>{x[0]}</a>)}</div>
    </nav>
    ...
    <section id="story"><p className="num">01 / THE STORY</p><blockquote>“When the system says denied, we create our own justice and peace.”</blockquote>
      <div className="columns"><h2>A story buried<br/>for more than<br/><i>three decades.</i></h2><div>
        ...
        <p><strong>Justice Denied</strong> is a 72-minute documentary following Jestina&apos;s fight for answers: exhumation, DNA testing, a second autopsy, and the right to bury her father with dignity—not in an unmarked pauper&apos;s grave.</p>
        <p>This is about what happens when official records conflict, institutions close ranks, and a daughter refuses to let silence become the final word. It is also about how she turned childhood grief into authorship, music, theater, film, and an enduring act of remembrance.</p>
      </div></div></section>

    <section className="dark" id="art"><Heading number="02 / REFRAMING JUSTICE THROUGH ART" title="When truth-telling becomes justice." text="When traditional systems failed to provide answers, Jestina turned to the tools she possessed as an artist." light/>
      <div className="columns"><h2>Gary is the reason.<br/>Truth is the mission.<br/><i>Art is the vehicle.</i></h2><div>
        <p>For nearly 33 years, Jestina Weems-Rosenduft has fought to keep her father Gary Jerome Weems&apos;s name, humanity, and story alive. Through film, fiction, theater, music, public speaking, social media, and advocacy, she has refused to let institutional silence erase him.</p>
        <p><strong>Justice Denied</strong> is more than a documentary about pursuing justice. It is an act of remembrance, resistance, and creative reclamation—preserving a family&apos;s truth and reframing justice as something families can also pursue through voice, creativity, community, and the refusal to let their loved ones be forgotten.</p>
        <p>Her work asks: When the system does not deliver answers, how can telling the truth become its own form of healing, accountability, and change?</p>
      </div></div>
    ...
    <section id="archive"><Heading number="04 / THE ARCHIVE" title="The record remains open." text="Explore documents, chronology, production history, and educational resources connected to the film."/>
      <div className="fileGrid">{files.map((f,i)=><article key={f[2]}><div className="filetop"><span>FILE 0{i+1}</span><span>{f[1]}</span></div><small>{f[0]}</small><h3>{f[2]}</h3><p>{f[3]}</p><a className="fileLink" href={`/pdf-viewer.html?file=${encodeURIComponent(f[4])}&title=${encodeURIComponent(f[2])}`}>OPEN PDF →</a></article>)}</div>
    ...
    <section className="dark" id="festivals"><Heading number="05 / FESTIVAL JOURNEY" title="Follow the film." text="Track submissions, selections, awards, and upcoming public screenings." light/>
    ...
    <section className="speaker" id="speak"><div className="portrait" role="img" aria-label="Portrait of Jestina Weems-Rosenduft"><small>JESTINA WEEMS-ROSENDUFT</small></div><div className="speakerCopy">
      <p className="num">06 / BRING THE STORY TO YOUR COMMUNITY</p><h2>Book Jestina<br/>to speak.</h2>
      <p>Jestina Weems-Rosenduft is an author, filmmaker, playwright, lyricist, producer, editor, transformational speaker, advocate, and homeschooling mother who turns lived experience into honest conversations about justice, resilience, creativity, faith, and finding your voice.</p>
      <p>Her programs explore turning pain into purpose, storytelling as a form of justice, reclaiming family narratives, and using art and authentic influence to build community and create change.</p>
      <ul><li>72-minute film screenings + Q&A</li><li>Turning Pain Into Purpose</li><li>Storytelling as a Form of Justice</li><li>Art, Advocacy + Authentic Influence</li><li>Schools, colleges, and libraries</li><li>Keynotes + community conversations</li></ul>
    ...
    <section className="support" id="support"><p className="num">07 / SUPPORT THE WORK</p><h2>Help carry this story<br/>into the world.</h2>
